...
public class Version_1_8_R1 extends Version {

    @Override
    public void send(final Player player, final String json) {
        try {
            final IChatBaseComponent iChatBaseComponent = ChatSerializer.a(ChatColor.translateAlternateColorCodes('&', json));
    ...
    public void send(final Player player, final List<String> json) {
        ...
        try {
            final IChatBaseComponent iChatBaseComponent = ChatSerializer.a(ChatColor.translateAlternateColorCodes('&', first));
            for (final String line : json) {
                if (line != null && !line.isEmpty()) {
                    if (line.equals(first)) {
                        continue;
                    }
                    iChatBaseComponent.addSibling(ChatSerializer.a(ChatColor.translateAlternateColorCodes('&', line)));
                }
            }
            final PacketPlayOutChat chat = new PacketPlayOutChat(iChatBaseComponent, (byte)0);
            ((CraftPlayer)player).getHandle().playerConnection.sendPacket(chat);
